...
from model.component import ControlKnotLieAlgebra, TransformationLieAlgebra
...
class Model(nerf.Model):
    ...
    def build_network(self, args, poses=None, event_poses=None):
        self.graph.evt_knot_pose_se3 = ControlKnotLieAlgebra(4)
        self.graph.rgb_knot_pose_se3 = ControlKnotLieAlgebra(4)
        self.graph.transform = TransformationLieAlgebra(1)
    ...
    def setup_optimizer(self, args):
        ...
        self.optim_nerf = torch.optim.Adam(params = grad_vars, lr = args.lrate)
        ...
        return self.optim_nerf, self.optim_pose, self.optim_transform, self.optim_rgb_crf, self.optim_event_crf
...
class Graph(nerf.Graph):
    def get_pose_evt(self, args, events_ts, seg_num = None):
        ...
        if args.traj == "linear":
            spline_poses = spline.linear_pose_unit_time(se3_knot_0, se3_knot_3, events_ts)
        elif args.traj == "spline":
            # interpolate pose at start and end of event time window
            spline_poses = spline.cubic_spline_pose_unit_time(
    ...
    def get_pose_rgb(self, args, exposure_ts, seg_num = None):
        ...
        pose_nums = None
        if seg_num is None:
            pose_nums = args.num_interpolated_pose
        else:
            pose_nums = seg_num
        ...
        if args.traj == "linear":
            spline_poses = spline.linear_pose_unit_time(
                se3_knot_0, se3_knot_3, exposure_ts
            )
        elif args.traj == "spline":
            spline_poses = spline.cubic_spline_pose_unit_time(
