...
def train(args):
    ...
    if args.dataset_type == 'llff':
        ...
        if images.shape[0] != poses.shape[0]:  # 代表每一张图不止对应一个 pose
            if images.shape[0] * (args.trajectory_seg_num + 1) == poses.shape[0]:
                ...
                print('!!! Initialize the SE3 Network with GT of start pose and end pose, the number of poses is ',
                      poses_start.shape[0])
            elif images.shape[0] == poses.shape[0] + 1:
                poses = torch.concatenate([poses[0].unsqueeze(0), poses],
                                          axis=0)  # initialize with poses corresponding to middle lines
            else:
                print(
    ...
    for frame_num in range(test_num):
        ...
        events, _, _, _, _, _ = load_test_data.load_llff_data(frame_num, args.datadir, args.threshold,
                                                              pose_state=load_state,
        ...
        for i in trange(start, N_iters):
            ...
            if i % args.i_video == 0 and i > 0:
                ret, ray_idx, test_poses, events_accu, ret_rgb, ray_idx_rgb = graph.forward(i, frame_id, threshold,
                                                                                            events, H, W, K, args)
                ...
                ret, ray_idx, events_accu, ret_rgb, ray_idx_rgb = graph.forward(i, frame_id, threshold, events, H, W, K,
                                                                                args)

            pixels_num = ray_idx.shape[0]

            ret_Gray1 = {'rgb_map': ret['rgb_map'][:pixels_num].clone().detach().requires_grad_(True),
                         'rgb0': ret['rgb0'][:pixels_num].clone().detach().requires_grad_(True)}
            ...
            if i % args.i_weights == 0 and i > 0:
                path = os.path.join(basedir, expname, 'frame_{:03d}.tar'.format(frame_num))
                torch.save({
            ...
            if i % args.i_img == 0 and i > 0:
                with torch.no_grad():
                    imgs_render = render_image_test(i, graph, test_poses, H, W, K, args,
                                                    dir='test_poses_mid_{:03d}'.format(frame_num), need_depth=False)
