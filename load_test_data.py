...
def _load_data(test_id, basedir, threshold, pose_state, factor=None, width=None, height=None, load_imgs=True,
               focal=None):
    ...
    events = np.load(os.path.join(basedir, 'test_data', 'event_threshold_' + '{:.1f}'.format(threshold),
                                  'gray_events_data_{:03d}.npy'.format(test_id)))
...
def load_llff_data(test_id, basedir, threshold, pose_state=None, factor=1, recenter=True, bd_factor=.75, spherify=False,
                   path_zflat=False, focal=None):
    # poses[3, 5, N] bds[2, N] imgs:images_factor [HWCN]
    poses, bds, imgs, events, poses_ts = _load_data(test_id, basedir, threshold, pose_state=pose_state, factor=factor,
