...
class Graph(nn.Module):
    ...
    def forward(self, i, poses_ts, threshold, events, H, W, K, args):
        ...
        if i <= threshold:
            ...
            if args.sliding_Win:
                ...
                window_low_bound = np.random.randint(events['num'] - N_window) // (N_window * args.chunk_interval) * (
                            N_window * args.chunk_interval)  # fixing window
                pose_i = window_low_bound // (N_window * args.chunk_interval)

            window_up_bound = int(window_low_bound + N_window * args.chunk_percent)
            ...
            events_ts = ts_window[np.array([0, N_window - 1])]
            ...
            spline_poses = self.get_pose(pose_i)
            ...
            if i % args.i_video == 0 and i > 0:
                pose_i = torch.arange(self.se3.end.weight.shape[0])
                test_poses = self.get_pose(pose_i)
    ...
    def get_pose(self, pose_i):  # 父类 get_pose 会被子类重写，所以后续调用的 get_pose 都为 cubic_spline 之后的 pose
